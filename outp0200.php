...
$cols = array(
   array("field"=>"cust_no" , "text"=>"客戶編號", "width"=>"80px" )
  ,array("field"=>"cust_na" , "text"=>"客戶簡稱", "foot"=>"小計/平均")
  ,array("field"=>"tel1"    , "text"=>"客戶電話" )
  ,array("field"=>"amt"     , "text"=>"出貨金額","class"=>"right", "format"=>"n:2", "foot"=>"sum")
...
$sql= "SELECT yymm,cust_no , SUM(amt) amt, SUM(amt1) amt1, SUM(amt)-SUM(amt1) amts
  , cusm0100.cust_abbr cust_na , cusm0100.tel1 , cusm0100.fsn
  FROM(
  (SELECT yymm,cust_no, SUM(amt) amt,0 amt1 FROM outh0100
  	WHERE  amt>0
  	AND yymm='$myymm'
  	GROUP BY yymm,cust_no)
  UNION
  (SELECT yymm,cust_no, 0 amt, SUM(amt)*-1 amt1 FROM outh0100
  	WHERE amt<0
  	AND yymm='$myymm'
  	GROUP BY yymm,cust_no)
  ) outh
...
if (!empty($data) && $mtran="Y") {
  ...
  foreach ($data as $row) {
    ...
    if (empty($mautoid)) {
      ...
      // 前期餘額 lst_amt
      if ($mtrlst=='Y') {
        $sqll = "SELECT bln_amt FROM $tbl WHERE cust_no='$mcust_no' AND yymm<'$myymm' ORDER BY yymm DESC LIMIT 1";
        if ($db1->dbQuery($sqll)) {
          $rl =$db1->dbRead(0);
          $lstamt = $rl['bln_amt'];
        }
      ...
      $blnrec = $blnamt ;
      $sql = "INSERT INTO $tbl(yymm,cust_no ,out_amt , bad_amt , sal_amt , tax_rate , tax_amt , lst_amt , bln_amt, bln_rec)
                        VALUES ('$myymm','$mcust_no', $wrkamt , $wrkbad , $salamt , $taxrate , $taxamt, $lstamt , $blnamt, $blnrec)";
    } else if (empty($mflag)) {
      $blnamt = $lstamt + $salamt + $taxamt;
      $blnrec = $blnamt - $recamt;
      $sql = "UPDATE $tbl SET wrk_amt=$wrkamt , wrk_bad=$wrkbad , sal_amt=$salamt , tax_amt=$taxamt, bln_amt=$blnamt
