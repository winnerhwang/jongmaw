...
class dbConnection {
    ...
    // -- Connect to the db
    public function connect($dbIP="localhost", $dbUser="root", $dbPass="winner", $dbName="jm", $dbPort = null) {
        // check parameters
        ...
        if ($this->dbType == 'mysql') {
            ...
            mysql_set_charset($this->dbCharacter);
...
//        $this->raise_error($this->res_errMsg);          // write errMsg to log file   ... by winner ...
        ...
        if ($this->res_errMsg != '') {
          $serror["error"] = $this->res_errMsg;
        } else {
          $serror["rowCount"] = $this->res_rowCount;
          $serror["fieldCount"] = $this->res_fieldCount;
        }
        ...
        elseif (substr($sql,0,6) == 'SELECT')
              $fhandle = fopen( 'log\select'.date('Ymd').'.txt', 'a' );
        else  $fhandle = fopen( 'log\access'.date('Ymd').'.txt', 'a' );
        if($fhandle){
