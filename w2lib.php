...
class w2grid_class {
    ...
    public function getRecords($sql, $cql, $request) {
        ...
        if ($cql == null || $cql == "") {
          ...
          $cql = str_ireplace("~order~", "~sort~", $cql);
...
//            $data['records'][$len]['recid'] = $len+1;
    ...
    public function getRecord($sql) {
        ...
        if ($db->res_errMsg != '') {
            $data = Array();
            $data['status'] = 'error';
            $data['message'] = $db->res_errMsg . $sql;
            return $data;
        }
        $data['status'] = 'success';
        $data['record']    = Array();
...
//            $sql = "UPDATE $table SET $values WHERE $keyField = '".addslashes($data['recid'])."'";
            $sql = "UPDATE $table SET $values WHERE $keyField = '".addslashes($data['record'][$keyField])."'";
        }
        // execute sql
        $rs = $db->execute($sql);
        // check for error
    ...
    public function getItems($sql) {
        ...
        while ($rs && !$rs->EOF) {
            ...
            $data['items'][$len]['recid'] = $len+1;
            $data['items'][$len]['recid'] = $rs->fields[0];
//            $data['items'][$len]['text'] = $rs->fields[0]." | ".$rs->fields[1];
//            $data['items'][$len]['text'] = $rs->fields[0];
            $data['items'][$len]['caption'] = $rs->fields[0]." | ".$rs->fields[1];
            foreach ($rs->fields as $k => $v) {
                if (intval($k) > 0 || $k == "0") continue;
                $data['items'][$len][$k] = $v;
            }
    ...
    public function updateSlave($sql) {   // by winner added
        global $db;
        // execute sql
        $rs = $db->execute($sql);
        // check for error
