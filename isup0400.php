...
$cols = array(
  ...
  ,array("field"=>"cust_no", "text"=>"廠商"     , "width"=>"40px")
  ,array("field"=>"cust_na", "text"=>"廠商簡稱" )
  ,array("field"=>"wkt_no" , "text"=>"加工"     , "width"=>"40px")
  ,array("field"=>"wkt_na" , "text"=>"加工名" )
...
$footLeft = "製表日期: ~~today~~ ";
$footMiddle=(empty($mcust_no)?"": "過濾廠商:".$mcust_no." - ".$_REQUEST['param']['cust_na']);
$footRight= "第 ~~curpg~~ ／ ~~pages~~ 頁";
$sqlc= (empty($mcust_no)?"": " AND isuh0100.cust_no='".$mcust_no."' ");
$sql= "SELECT isuh0100.date , isuh0100.cust_no
        ...
        LEFT OUTER JOIN venm0100 ON isuh0100.cust_no=venm0100.cust_no)
        LEFT OUTER JOIN wktm0100 USING(wkt_no)
        WHERE isui0100.stk_no = '$mstk_no'
