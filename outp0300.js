      ...
      ,onChange: function(event)  {
          switch(event.target)  {
            case "cust_no"  :
                ...
                formc.recid=formc.postData['cust_no']= formp.record['cust_no'];
                formc.reload( function(data){
                  if (data.status="success") {
                    consoleLog("set fullname",data,formc);
                    formp.record['cust_fn']=formp.get('cust_fn').el.value= formc.record['cust_name'];
                    formp.record['tel1']   =formp.get('tel1'   ).el.value= formc.record['tel1'];
                    formp.record['fax']    =formp.get('fax'    ).el.value= formc.record['fax'];
                    formp.record['addr1']  =formp.get('addr1'  ).el.value= formc.record['co_addr1'];
